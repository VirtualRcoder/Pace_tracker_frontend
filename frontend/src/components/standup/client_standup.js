...
import Standup from './display_client_standup';
          ...
          <center>
        ...
        </Jumbotron>
        	{
        		standups.length > 0 ? standups.map((standupsitem, index) =>{
		            if(standupsitem.project == id)
	            {
		              return(
                    <Standup project={standupsitem.project} created_by={standupsitem.created_by} next={standupsitem.what_is_next} done={standupsitem.what_is_done} blocking={standupsitem.what_is_blocking} note={standupsitem.note} modified={standupsitem.modified_at}/>
                  )
		            }
        		}): <p>Not found</p>
        	}    
